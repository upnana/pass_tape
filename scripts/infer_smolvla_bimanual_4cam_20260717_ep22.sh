#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual 4cam 20260717_150424 · ~22 epoch（ckpt 060000）
#
# 默认用 checkpoints/060000（≈20.3 epoch；精确22≈65k无存盘，070000≈23.7）
# 训练仍在跑时请换空闲 GPU，或与训练共享卡（会变慢）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_4cam_20260717_ep22.sh offline
#   bash infer_smolvla_bimanual_4cam_20260717_ep22.sh robot
#   bash infer_smolvla_bimanual_4cam_20260717_ep22.sh cameras
#   bash infer_smolvla_bimanual_4cam_20260717_ep22.sh viz
#
# 换 ckpt:
#   CHECKPOINT=.../checkpoints/070000/pretrained_model bash infer_smolvla_bimanual_4cam_20260717_ep22.sh offline
# =============================================================================
set -euo pipefail
...
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/060000/pretrained_model}"  # ~22 ep → 060000≈20.3
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_4cam_20260717_ep22}"
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_4cam_20260717_ep22" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep22 / 060000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_4cam_20260717_ep22.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_4cam_20260717_ep22" \
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_4cam_20260717_ep22.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
