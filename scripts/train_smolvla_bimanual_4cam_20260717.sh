#!/bin/bash
# =============================================================================
# SmolVLA 训练：bimanual handover 4cam（20260717_150424）
#
# Dataset: /home/rxn/datasets/bimanual_handover_4cam_20260717_150424
#   ~89 ep / ~47k frames / 12-DoF / 4 cameras
#
# Usage:
#   conda activate lerobot
#   bash train_smolvla_bimanual_4cam_20260717.sh              # 单卡 GPU1 前台
#   BACKGROUND=1 bash train_smolvla_bimanual_4cam_20260717.sh # 后台
#   RESUME=1 bash train_smolvla_bimanual_4cam_20260717.sh     # 从 last 续训
#   FRESH=1 bash train_smolvla_bimanual_4cam_20260717.sh      # 删 output 重训
#   CUDA_VISIBLE_DEVICES=0 bash train_smolvla_bimanual_4cam_20260717.sh
#   tail -f logs/smolvla_bimanual_4cam_20260717_150424.log
#
# Steps 说明:
#   ~47074 frames @ bs=4 → ~11769 steps/epoch
#   STEPS=200000 ≈ 17 epoch；不够再 RESUME 加长
# =============================================================================
set -euo pipefail
...
JOB_NAME="${JOB_NAME:-smolvla_bimanual_4cam_20260717_150424}"
...
STEPS="${STEPS:-200000}"        # ≈ 17 epoch @ bs=4
...
if [ "${BACKGROUND:-0}" = "1" ] && [ -z "${TRAIN_SMOLVLA_BIMANUAL_4CAM_BG:-}" ]; then
    ...
    echo "  log: ${LOG_FILE}"
    nohup env TRAIN_SMOLVLA_BIMANUAL_4CAM_BG=1 \
...
echo "=============================="
echo "SmolVLA Bimanual 4cam (20260717_150424)"
echo "=============================="
...
[ "${NUM_GPUS}" -gt 1 ] && ACCEL_ARGS+=(--multi_gpu --main_process_port="${MAIN_PROCESS_PORT:-29505}")
