#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual 4cam 20260717_150424 · ~25 epoch（ckpt 070000）
#
# 默认用 checkpoints/070000（≈23.7 epoch；最接近第25的保存点。080000≈27）
# 训练仍在跑时请换空闲 GPU，或与训练共享卡（会变慢）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_4cam_20260717_ep25.sh offline
#   bash infer_smolvla_bimanual_4cam_20260717_ep25.sh robot
#   bash infer_smolvla_bimanual_4cam_20260717_ep25.sh cameras
#   bash infer_smolvla_bimanual_4cam_20260717_ep25.sh viz
#
# 换 ckpt:
#   CHECKPOINT=.../checkpoints/080000/pretrained_model bash infer_smolvla_bimanual_4cam_20260717_ep25.sh offline
# =============================================================================
set -euo pipefail
...
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/070000/pretrained_model}"  # ~25 ep 最近保存点 (070000≈23.7)
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_4cam_20260717_ep25}"
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_4cam_20260717_ep25" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep25 / 070000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_4cam_20260717_ep25.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_4cam_20260717_ep25" \
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_4cam_20260717_ep25.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
