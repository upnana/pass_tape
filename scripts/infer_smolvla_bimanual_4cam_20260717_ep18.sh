#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual 4cam 20260717_150424 · ~18 epoch（ckpt 050000）
#
# 默认用 checkpoints/050000（保存点 ≈16.9–17.0 epoch，最接近第 18 epoch）。
# 训练仍在跑时请换空闲 GPU，例如 CUDA_VISIBLE_DEVICES=0（若 0/1 都被占则等/停训）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_4cam_20260717_ep18.sh offline
#   bash infer_smolvla_bimanual_4cam_20260717_ep18.sh robot
#   bash infer_smolvla_bimanual_4cam_20260717_ep18.sh cameras
#   bash infer_smolvla_bimanual_4cam_20260717_ep18.sh viz
#
# 换更晚的 ckpt:
#   CHECKPOINT=.../checkpoints/060000/pretrained_model bash infer_smolvla_bimanual_4cam_20260717_ep18.sh offline
# =============================================================================
set -euo pipefail
...
DATA_ROOT="${DATA_ROOT:-/home/rxn/datasets/bimanual_handover_4cam_20260717_150424}"
DATA_REPO="${DATA_REPO:-my_bimanual/handover_4cam_20260717_150424}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/train/smolvla_bimanual_4cam_20260717_150424}"
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/050000/pretrained_model}"  # ~17 ep ≈ 第18 epoch 最近保存点
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_4cam_20260717_ep18}"
...
echo "=============================="
echo "SmolVLA Bimanual 4cam ep≈18 Inference (${MODE})"
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_4cam_20260717_ep18" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep18 / 050000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_4cam_20260717_ep18.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_4cam_20260717_ep18" \
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_4cam_20260717_ep18.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
