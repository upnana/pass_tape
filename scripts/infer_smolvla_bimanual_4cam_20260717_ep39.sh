#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual 4cam 20260717_150424 · ~39 epoch（ckpt 110000）
#
# 默认用 checkpoints/110000（≈39 epoch，当前 last）。
# 训练仍在跑时请换空闲 GPU，或与训练共享卡（会变慢）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_4cam_20260717_ep39.sh offline
#   bash infer_smolvla_bimanual_4cam_20260717_ep39.sh robot
#   bash infer_smolvla_bimanual_4cam_20260717_ep39.sh cameras
#   bash infer_smolvla_bimanual_4cam_20260717_ep39.sh viz
#
# 换更晚的 ckpt:
#   CHECKPOINT=.../checkpoints/110000/pretrained_model bash infer_smolvla_bimanual_4cam_20260717_ep39.sh offline
# =============================================================================
set -euo pipefail
...
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/110000/pretrained_model}"  # ~39 ep 最近保存点
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_4cam_20260717_ep39}"
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_4cam_20260717_ep39" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep39 / 110000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_4cam_20260717_ep39.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_4cam_20260717_ep39" \
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_4cam_20260717_ep39.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
